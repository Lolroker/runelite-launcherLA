...
APPBASE="build/macos-x64/Luna.app"
...
build() {
    ...
    cp native/build-x64/src/Luna $APPBASE/Contents/MacOS/
...
SIGNING_IDENTITY="Developer ID Application: Arthur Wagterveld (9465G4SH99)"
    ...
    mv Luna\ *.dmg Luna-x64.dmg

    # dump for CI
    hdiutil imageinfo Luna-x64.dmg

    if ! hdiutil imageinfo Luna-x64.dmg | grep -q "Format: UDBZ" ; then
        echo "Format of resulting dmg was not UDBZ, make sure your create-dmg has support for --format"
        exit 1
    fi

    if ! hdiutil imageinfo Luna-x64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit Luna-x64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple Luna-x64.dmg
    fi
}
