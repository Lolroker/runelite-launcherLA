...
APPBASE="build/macos-aarch64/Luna.app"

build() {
    echo Launcher sha256sum
    shasum -a 256 build/libs/Luna.jar
    ...
    cp native/build-aarch64/src/Luna $APPBASE/Contents/MacOS/
    cp build/libs/Luna.jar $APPBASE/Contents/Resources/
    ...
    echo Setting world execute permissions on Luna
    pushd $APPBASE
    chmod g+x,o+x Contents/MacOS/Luna
    popd

    otool -l $APPBASE/Contents/MacOS/Luna
}
...
dmg() {
    ...
    mv Luna\ *.dmg Luna-aarch64.dmg

    # dump for CI
    hdiutil imageinfo Luna-aarch64.dmg

    if ! hdiutil imageinfo Luna-aarch64.dmg | grep -q "Format: ULFO" ; then
        echo Format of dmg is not ULFO
        exit 1
    fi

    if ! hdiutil imageinfo Luna-aarch64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit Luna-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple Luna-aarch64.dmg
    fi
}
